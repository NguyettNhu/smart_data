...
function generateSkeleton(isFall: boolean, baseX: number, baseY: number): { x: number; y: number }[] {
  if (isFall) {
    // Lying down skeleton
    return [
  ...
  const generateDetections = useCallback((): Detection[] => {
    ...
    for (let i = 0; i < personCount; i++) {
      ...
      const detection: Detection = {
        ...
        skeleton: showSkeleton ? generateSkeleton(isFall, 100 + (i * 200), isFall ? 280 : 150) : undefined,
      };
